var procesoTipoAsignacion = {
    ...
    //Metodo Inicial
    init: function () {
        procesoTipoAsignacion.cargaDatos();
        procesoTipoAsignacion.validaTeclado();
        procesoTipoAsignacion.procesoGuardado();
        procesoTipoAsignacion.procesoMuestra();
        procesoTipoAsignacion.validaDeshabilitar();
        procesoTipoAsignacion.procesoLimpiar();

    },// fin init
    cargaDatos:function () {
        $(document).ready( function () {
            procesoTipoAsignacion.table = $('#dt_tabla').DataTable( {
                ...
                ajax: {
                    url: baseUrl + "/controlactivos/index/obtener-json-catalogo/tipoCatalogo/TIPO_ASIGNACION",
                    dataSrc: 'data'
                },
                columns: [
            ...
            procesoTipoAsignacion.table.columns().every( function () {
                var that = this;
                ...
                , submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            procesoTipoAsignacion.metodoMensaje(data, 'btnGuardar', procesoTipoAsignacion.mensajeExito01, procesoTipoAsignacion.mensajePeligro01, 'msjRespuesta' );
                        }
                    });
                },
    ...
    validaDeshabilitar: function () {
        $(document).on("click", ".btnAccionDel", function () {
            ...
            procesoTipoAsignacion.modalMensaje('Confirmación', '¿ Está seguro de deshabilitar ?', id, 'btnAccionDel');
    ...
    procesoDeshabilitar: function (valor) {
        $.ajax({
            ...
            success: function (data) {
                //Refresco Datatable
                procesoTipoAsignacion.table.ajax.reload();
    ...
    metodoMensaje: function (data, bonton, mensajeExito, mensajeFalse, divResp) {
        if (data.valida == 'true') {
            var str = procesoTipoAsignacion.EstruturaMesajeExito;
        ...
        if (data.valida == 'false') {
            var str = procesoTipoAsignacion.EstruturaMesajePeligro;
            var respHtml = str.replace("i_mensaje", mensajeFalse);
            $("." + divResp).html(respHtml);
        }
        procesoTipoAsignacion.funtCerrarReload();
    },
    validaTeclado: function () {
        var validaCamposTeclado = $(function () {
            $(".soloLetrasNum").keypress(function (e) {
                procesoTipoAsignacion.metodoTeclado(e, "soloLetrasNum", this);
            });
        });
    ...
    metodoTeclado: function (e, permitidos, fieldObj, upperCase) {
        ...
        if ((procesoTipoAsignacion.is_nonChar(charCode)) && e.shiftKey == 0)
            return true;
        else if (charCode == '')
            charCode = e.charCode;
    ...
    modalMensaje: function (titulo1, mensaje, valor, proceso) {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                autospin: false,
                action: function (dialog) {
                    procesoTipoAsignacion.procesoDeshabilitar(valor);
                    dialog.close();
                }
            }]
        });
