var procesoCategoriaActivo = {
    ...
    //Metodo Inicial
    init: function () {
        procesoCategoriaActivo.cargaDatos();
        procesoCategoriaActivo.validaTeclado();
        procesoCategoriaActivo.procesoGuardado();
        procesoCategoriaActivo.procesoMuestra();
        procesoCategoriaActivo.validaDeshabilitar();
        procesoCategoriaActivo.procesoLimpiar();

    },// fin init
    cargaDatos:function () {
        $(document).ready( function () {
            procesoCategoriaActivo.table =  $('#dt_tabla').DataTable( {
            ...
            procesoCategoriaActivo.table.columns().every( function () {
                var that = this;
                ...
                , submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            procesoCategoriaActivo.metodoMensaje(data, 'btnGuardar', procesoCategoriaActivo.mensajeExito01, procesoCategoriaActivo.mensajePeligro01, 'msjRespuesta');
                        }
                    });
                },
    ...
    validaDeshabilitar: function () {
        $(document).on("click", ".btnAccionDel", function () {
            ...
            procesoCategoriaActivo.modalMensaje('Confirmación', '¿ Está seguro de deshabilitar ?', id, 'btnAccionDel');
    ...
    procesoDeshabilitar: function (valor) {
        $.ajax({
            ...
            success: function (data) {
                //Refresco Datatable
                procesoCategoriaActivo.table.ajax.reload();
    ...
    metodoMensaje: function (data, bonton, mensajeExito, mensajeFalla, didResp ) {
        console.log(data);
        if (data.valida == 'true') {
            var str = procesoCategoriaActivo.EstruturaMesajeExito;
        ...
        if (data.valida == 'false') {
            var str = procesoCategoriaActivo.EstruturaMesajePeligro;
    ...
    validaTeclado: function () {
        var validaCamposTeclado = $(function () {
            $(".soloLetrasNum").keypress(function (e) {
                procesoCategoriaActivo.metodoTeclado(e, "soloLetrasNum", this);
            });
        });
    ...
    metodoTeclado: function (e, permitidos, fieldObj, upperCase) {
        ...
        if ((procesoCategoriaActivo.is_nonChar(charCode)) && e.shiftKey == 0)
            return true;
        else if (charCode == '')
            charCode = e.charCode;
    ...
    modalMensaje: function (titulo1, mensaje, valor, proceso) {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                autospin: false,
                action: function (dialog) {
                    procesoCategoriaActivo.procesoDeshabilitar(valor);
                    dialog.close();
