var procesoFisico = {
    ...
    //Metodo Inicial
    init: function () {
        procesoFisico.cargaDatos();
        procesoFisico.validaTeclado();
        procesoFisico.procesoGuardado();
        procesoFisico.procesoMuestra();
        procesoFisico.validaDeshabilitar();
        procesoFisico.procesoLimpiar();

    },// fin init
    cargaDatos:function () {
        $(document).ready( function () {
            procesoFisico.table =  $('#dt_table').DataTable( {
            ...
            procesoFisico.table.columns().every( function () {
                var that = this;
                ...
                , submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            procesoFisico.metodoMensaje(data, 'btnGuardar', procesoFisico.mensajeExito01, procesoFisico.mensajePeligro01, 'msjRespuesta');
                        }
                    });
                },
    ...
    validaDeshabilitar: function () {
        $(document).on("click", ".btnAccionDel", function () {
            ...
            procesoFisico.modalMensaje('Confirmación', '¿ Está seguro de deshabilitar ?', id, 'btnAccionDel');
    ...
    procesoDeshabilitar: function (valor) {
        $.ajax({
            ...
            success: function (data) {
                //Refresco Datatable
                procesoFisico.table.ajax.reload();
            }
        });
    ...
    metodoMensaje: function (data, bonton, mensajeExito, mensajeFalla, divMensaje) {
        console.log(data);
        if (data.valida == 'true') {
            var str = procesoFisico.EstruturaMesajeExito;
        ...
        if (data.valida == 'false') {
            var str = procesoFisico.EstruturaMesajePeligro;
    ...
    validaTeclado: function () {
        var validaCamposTeclado = $(function () {
            $(".soloLetrasNum").keypress(function (e) {
                procesoFisico.metodoTeclado(e, "soloLetrasNum", this);
            });
        });
    ...
    metodoTeclado: function (e, permitidos, fieldObj, upperCase) {
        ...
        if ((procesoFisico.is_nonChar(charCode)) && e.shiftKey == 0)
            return true;
        else if (charCode == '')
            charCode = e.charCode;
    ...
    modalMensaje: function (titulo1, mensaje, valor, proceso) {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                autospin: false,
                action: function (dialog) {
                    procesoFisico.procesoDeshabilitar(valor);
                    dialog.close();
                }
            }]
        });
