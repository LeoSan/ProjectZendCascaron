var procesoMotivo = {
    ...
    //Metodo Inicial
    init: function () {
        procesoMotivo.cargaDatos();
        procesoMotivo.validaTeclado();
        procesoMotivo.procesoGuardado();
        procesoMotivo.procesoMuestra();
        procesoMotivo.validaDeshabilitar();
        procesoMotivo.procesoLimpiar();

    },// fin init
    cargaDatos:function () {
        $(document).ready( function () {
            procesoMotivo.table =  $('#dt_table').DataTable( {
            ...
            $('#dt_table thead th').each( function (i) {
                if (i <($('#dt_table thead th').length - 1) ){
                    var title = $(this).text();
                    $(this).html( '<input type="text" class="form-control" placeholder=" '+title+'" />' );
                }
            });
            procesoMotivo.table.columns().every( function () {
                var that = this;
                ...
                , submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            procesoMotivo.metodoMensaje(data, 'btnGuardar', procesoMotivo.mensajeExito01, procesoMotivo.mensajePeligro01, 'msjRespuesta');
                        }
                    });
                },
    ...
    validaDeshabilitar: function () {
        $(document).on("click", ".btnAccionDel", function () {
            ...
            procesoMotivo.modalMensaje('Confirmación', '¿ Está seguro de deshabilitar ?', id, 'btnAccionDel');
    ...
    procesoDeshabilitar: function (valor) {
        $.ajax({
            ...
            success: function (data) {
                //Refresco Datatable
                procesoMotivo.table.ajax.reload();
            }
        });
    ...
    metodoMensaje: function (data, bonton, mensajeExito, mensajeFalla, divMensaje) {
        console.log(data);
        if (data.valida == 'true') {
            var str = procesoMotivo.EstruturaMesajeExito;
        ...
        if (data.valida == 'false') {
            var str = procesoMotivo.EstruturaMesajePeligro;
    ...
    validaTeclado: function () {
        var validaCamposTeclado = $(function () {
            $(".soloLetrasNum").keypress(function (e) {
                procesoMotivo.metodoTeclado(e, "soloLetrasNum", this);
            });
        });
    ...
    metodoTeclado: function (e, permitidos, fieldObj, upperCase) {
        ...
        if ((procesoMotivo.is_nonChar(charCode)) && e.shiftKey == 0)
            return true;
        else if (charCode == '')
            charCode = e.charCode;
    ...
    modalMensaje: function (titulo1, mensaje, valor, proceso) {
        ...
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                autospin: false,
                action: function (dialog) {
                    procesoMotivo.procesoDeshabilitar(valor);
                    dialog.close();
                }
            }]
        });
